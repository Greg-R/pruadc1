...
\begin{enumerate}
\item  Flash Beaglebone image to micro-sd.
...
\item  ssh root@192.168.1.7 (or whatever the IP is set to).  If you are using a router with a GUI control application, it may have a display which indicates the board is connected and which IP address has been assigned to it.
...
However, the clpru compiler binary is not there.  Run this command:
\begin{verbatim}
which clpru
\end{verbatim}
...
/usr/bin/clpru
\end{verbatim}
So the compiler binary is in a different location.  This is a problem for the lab's make files.  Execute the following commands (as root):
\begin{verbatim}
...
make
\end{verbatim}
This will fail, it is looking for environment variable \$PRU\_CGT.  Execute:
...
\end{verbatim}
\item  Now execute almost the same as \#9, this time for pru1:
\begin{verbatim}
cd gen
